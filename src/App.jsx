...
function generarBloques(fecha, duracionMin) {
  ...
  while (mins + duracionMin <= fin) {
    ...
    bloques.push({
      ...
      fin: `${String(hf).padStart(2, "0")}:${String(mf).padStart(2, "0")}`,
    });
    mins += 30;
  }
  return bloques;
...
const labelFecha = (f) =>
  new Date(f + "T12:00:00").toLocaleDateString("es-MX", { weekday: "short", day: "numeric", month: "short" });
const colorTipo = (tipo) => TIPOS_SERVICIO.find((t) => t.id === tipo)?.color || "#2721E8";

// ─── MODAL AGENDAR CITA (reutilizable) ───────────────────────────────────────
// Se usa tanto desde la Agenda como después de cerrar ticket en POS
function ModalAgendarCita({ session, onClose, onCitaGuardada, clientaPrefill }) {
  const [busqueda, setBusqueda] = useState(clientaPrefill?.nombre || "");
  const [clientasEncontradas, setClientasEncontradas] = useState([]);
  const [clientaSel, setClientaSel] = useState(clientaPrefill || null);
  ...
  const [esNueva, setEsNueva] = useState(!clientaPrefill);
  const [nombreNueva, setNombreNueva] = useState("");
  const [telefonoNueva, setTelefonoNueva] = useState("");
  const [saving, setSaving] = useState(false);
  const [citas, setCitas] = useState([]);
  // Si viene con clienta prefill, saltar directo al paso 2
  const [paso, setPaso] = useState(clientaPrefill ? 2 : 1);

  // Cargar citas del día seleccionado para detectar ocupados
  ...
  useEffect(() => {
    if (tipoSvc && fechaCita) {
      setBloques(generarBloques(fechaCita, tipoSvc.duracion));
      setBloqueSel(null);
    }
  }, [tipoSvc, fechaCita]);

  // Cargar paquetes si viene con clienta prefill
  useEffect(() => {
    if (clientaPrefill?.id) {
      (async () => {
        const { data } = await supabase
          .from("paquetes")
          .select("*")
          .eq("clienta_id", clientaPrefill.id)
          .eq("activo", true);
        setPaquetesSel(data || []);
      })();
    }
  ...
  const esBloqueOcupado = (b) =>
    citas.some(
      (c) =>
        (b.inicio >= c.hora_inicio && b.inicio < c.hora_fin) ||
        (b.fin > c.hora_inicio && b.fin <= c.hora_fin)
    );
    ...
    try {
      ...
      if (esNueva && !clientaSel) {
        const { data: newC } = await supabase
          .from("clientas")
          .insert([{ nombre: nombreNueva, telefono: telefonoNueva, sucursal_id: session.id, sucursal_nombre: session.nombre }])
          .select();
        clientaId = newC[0].id;
        clientaNombre = newC[0].nombre;
      }
      ...
      await supabase.from("citas").insert([{
        ...
        servicio: paqueteElegido?.servicio || tipoSvc.label,
        tipo_servicio: tipoSvc.id,
        duracion_min: tipoSvc.duracion,
        fecha: fechaCita,
        hora_inicio: bloqueSel.inicio,
        hora_fin: bloqueSel.fin,
      ...
      if (paqueteElegido) {
        ...
        await supabase
          .from("paquetes")
          .update({ sesiones_usadas: ns, activo: ns < paqueteElegido.total_sesiones })
          .eq("id", paqueteElegido.id);
      }
      onCitaGuardada?.();
      onClose();
    } catch (e) {
      ...
      <div className="glass" style={{ width: 500, maxHeight: "88vh", overflow: "auto", padding: "26px" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "22px" }}>
          {[{ n: 1, l: "Clienta" }, { n: 2, l: "Servicio" }, { n: 3, l: "Horario" }].map((p, i) => (
            <div key={p.n} style={{ display: "flex", alignItems: "center", gap: "8px", flex: i < 2 ? 1 : "auto" }}>
              <div className="paso-ind" style={{
                background: paso >= p.n ? "#2721E8" : "rgba(255,255,255,0.06)",
                color: paso >= p.n ? "#fff" : "rgba(255,255,255,0.3)",
                flexShrink: 0,
              }}>{p.n}</div>
          ...
          <button onClick={onClose} style={{ background: "none", border: "none", color: "rgba(255,255,255,0.4)", cursor: "pointer", fontSize: "20px", marginLeft: "auto" }}>×</button>
        </div>

        {/* Banner si viene del POS */}
        {clientaPrefill && paso === 2 && (
          <div style={{
            padding: "12px 16px", marginBottom: "16px", borderRadius: "10px",
            background: "rgba(39,33,232,0.1)", border: "1px solid rgba(39,33,232,0.3)",
            display: "flex", alignItems: "center", gap: "10px",
          }}>
            <div style={{ fontSize: "18px" }}>📅</div>
            <div>
              <div style={{ fontSize: "13px", fontWeight: 600 }}>Agendar cita para {clientaPrefill.nombre}</div>
              <div style={{ fontSize: "11px", color: "rgba(255,255,255,0.4)" }}>Ticket cerrado — ahora selecciona servicio y horario</div>
            </div>
          </div>
        )}
                      ...
                      <div>
                        ...
                        {paquetesSel.map((p) => (
                          <div key={p.id} className="paq-card"
                            ...
                            {paqueteElegido?.id === p.id && (
                              <div style={{ fontSize: "10px", color: p.sesiones_usadas === 0 ? "#f0c040" : "#10b981", fontWeight: 600, marginTop: "4px" }}>
        ...
        {paso === 2 && (
          <div>
            ...
            <div style={{ fontSize: "11px", color: "rgba(255,255,255,0.4)", marginBottom: "14px" }}>
              {paqueteElegido ? `Paquete: ${paqueteElegido.servicio}` : clientaPrefill ? `Clienta: ${clientaPrefill.nombre}` : "Nueva clienta"}
            </div>

            {/* Mostrar paquetes activos si la clienta los tiene */}
            {clientaPrefill && paquetesSel.length > 0 && !paqueteElegido && (
              <div style={{ marginBottom: "14px" }}>
                <div style={{ fontSize: "10px", color: "rgba(255,255,255,0.4)", marginBottom: "8px", letterSpacing: "1px" }}>PAQUETES ACTIVOS DE {clientaPrefill.nombre.toUpperCase()}</div>
                {paquetesSel.map((p) => (
                  <div key={p.id} className="paq-card"
                ...
                <div style={{ fontSize: "10px", color: "rgba(255,255,255,0.3)", marginBottom: "8px" }}>O selecciona un tipo de servicio manualmente:</div>
              </div>
            )}

            {paqueteElegido && (
              <div style={{ marginBottom: "12px", padding: "10px 14px", background: "rgba(73,184,211,0.08)", border: "1px solid rgba(73,184,211,0.2)", borderRadius: "8px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <div>
                  <div style={{ fontSize: "12px", fontWeight: 600, color: "#49B8D3" }}>{paqueteElegido.servicio}</div>
                  <div style={{ fontSize: "10px", color: "rgba(255,255,255,0.4)" }}>Sesión {paqueteElegido.sesiones_usadas + 1} de {paqueteElegido.total_sesiones}</div>
                </div>
                <button onClick={() => { setPaqueteElegido(null); setTipoSvc(null); }} style={{ background: "none", border: "none", color: "rgba(255,255,255,0.3)", cursor: "pointer", fontSize: "14px" }}>×</button>
              </div>
            )}
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(2,1fr)", gap: "8px", marginBottom: "16px" }}>
              {TIPOS_SERVICIO.map((t) => (
                <div key={t.id} className="tipo-btn"
                  style={{ borderColor: tipoSvc?.id === t.id ? t.color : "rgba(255,255,255,0.08)", background: tipoSvc?.id === t.id ? `${t.color}15` : "rgba(0,0,0,0.2)", padding: "10px 14px", borderRadius: "10px", cursor: "pointer" }}
            ...
            <div style={{ display: "flex", gap: "8px" }}>
              {!clientaPrefill && <button className="btn-ghost" style={{ flex: 1 }} onClick={() => setPaso(1)}>← Atrás</button>}
              {clientaPrefill && <button className="btn-ghost" style={{ flex: 1 }} onClick={onClose}>Omitir</button>}
        ...
        {paso === 3 && (
          <div>
            ...
            {new Date(fechaCita + "T12:00:00").getDay() === 0 && (
              <div style={{ fontSize: "12px", color: "#ff6b6b", marginBottom: "10px" }}>⚠ Domingo — sucursal cerrada</div>
            )}
            {bloques.length > 0 && (
              <div>
                  ...
                  {bloques.map((b) => {
                    ...
                    return (
                      <div key={b.inicio} className="bloque-btn"
                        style={{
                          borderColor: ocupado ? "rgba(255,80,80,0.25)" : selec ? "#2721E8" : "rgba(255,255,255,0.08)",
                          background: ocupado ? "rgba(255,80,80,0.04)" : selec ? "rgba(39,33,232,0.2)" : "rgba(0,0,0,0.2)",
                          color: ocupado ? "rgba(255,80,80,0.4)" : selec ? "#fff" : "rgba(255,255,255,0.5)",
...
function Agenda({ session }) {
  ...
  const cargarCitas = async () => {
    const { data } = await supabase
      ...
      .eq("sucursal_id", session.id)
      .gte("fecha", semana[0])
      .lte("fecha", semana[semana.length - 1])
      .order("hora_inicio");
    if (data) setCitas(data);
  };
  ...
  const semanaAnterior = () => {
    const d = new Date(semana[0] + "T12:00:00");
    d.setDate(d.getDate() - 7);
    setSemana(semanaDesde(d.toISOString().slice(0, 10)));
  };
...
export default function CirePOS() {
  ...
  // Estado para el modal de agendar después de cerrar ticket
  const [showAgendarPostTicket, setShowAgendarPostTicket] = useState(false);
  const [clientaParaAgendar, setClientaParaAgendar]       = useState(null);
  ...
  const logout = () => {
    setSession(null); setLoginUser(""); setLoginPass(""); setTickets([]); resetForm();
  };
  ...
  const cerrarTicket = async () => {
    ...
    const nombreClienta = clienta || "Sin nombre";
    const nuevoTicket = {
      ticket_num: ticketNum, sucursal_id: session.id, sucursal_nombre: session.nombre,
      clienta: nombreClienta, tipo_clienta: tipoClienta,
    ...
    if (!error && data) {
      setLastTicket({ ...nuevoTicket });
      setTickets((prev) => [data[0], ...prev]);

      // ── FLUJO INTEGRADO: buscar/crear clienta y abrir agenda ──
      let clientaObj = null;
      if (nombreClienta !== "Sin nombre") {
        // Buscar si ya existe
        const { data: existentes } = await supabase
          .from("clientas")
          .select("*")
          .ilike("nombre", nombreClienta)
          .eq("sucursal_id", session.id)
          .limit(1);

        if (existentes && existentes.length > 0) {
          clientaObj = existentes[0];
        } else {
          // Crear nueva clienta
          const { data: nuevaClienta } = await supabase
            .from("clientas")
            .insert([{ nombre: nombreClienta, sucursal_id: session.id, sucursal_nombre: session.nombre }])
            .select();
          if (nuevaClienta && nuevaClienta.length > 0) {
            clientaObj = nuevaClienta[0];
          }
        }
      }

      setClientaParaAgendar(clientaObj);
      setShowAgendarPostTicket(true);
    }
    ...
    <div style={{ minHeight: "100vh", background: "#0C0D1A", fontFamily: "'Albert Sans', sans-serif", color: "#fff", display: "flex", flexDirection: "column" }}>
      <style>{`
        ...
        * { box-sizing:border-box; margin:0; padding:0; }
        ...
        .inp { background:rgba(255,255,255,0.05); border:1px solid rgba(255,255,255,0.1); border-radius:10px; padding:10px 14px; color:#fff; font-family:'Albert Sans',sans-serif; font-size:13px; width:100%; outline:none; transition:border 0.2s; }
      ...
      {/* TOAST */}
      {lastTicket && !showAgendarPostTicket && (
          ...
          <div style={{ borderLeft: "1px solid rgba(255,255,255,0.06)", padding: "20px", overflowY: "auto", background: "rgba(0,0,0,0.2)", display: "flex", flexDirection: "column", gap: "14px" }}>
            ...
            <div className="glass-dark" style={{ padding: "14px", display: "flex", flexDirection: "column", gap: "10px" }}>
              ...
              <div>
                ...
                <div style={{ display: "flex", gap: "6px" }}>
                  {[0, 5, 10, 15, 20].map((d) => (
                    <div key={d} onClick={() => setDescuento(d)} style={{
                      flex: 1, textAlign: "center", padding: "8px 4px", borderRadius: "8px", fontSize: "12px", fontWeight: 600, cursor: "pointer", border: "1px solid", transition: "all 0.15s",
      ...
      {showConfirm && (
        <div className="overlay">
          <div className="glass" style={{ width: 420, padding: "32px", borderColor: "rgba(39,33,232,0.4)" }}>
            ...
            {/* Indicador de que se abrirá la agenda */}
            {clienta && (
              <div style={{
                padding: "10px 14px", marginBottom: "16px", borderRadius: "8px",
                background: "rgba(73,184,211,0.08)", border: "1px solid rgba(73,184,211,0.2)",
                display: "flex", alignItems: "center", gap: "8px",
              }}>
                <span style={{ fontSize: "16px" }}>📅</span>
                <span style={{ fontSize: "11px", color: "#49B8D3" }}>Al confirmar podrás agendar la cita de {clienta}</span>
              </div>
            )}
      ...
      {/* MODAL POST-TICKET: Agendar cita automáticamente */}
      {showAgendarPostTicket && !showConfirm && clientaParaAgendar && (
        <ModalAgendarCita
          session={session}
          clientaPrefill={clientaParaAgendar}
          onClose={() => {
            setShowAgendarPostTicket(false);
            setClientaParaAgendar(null);
          }}
          onCitaGuardada={() => {
            setShowAgendarPostTicket(false);
            setClientaParaAgendar(null);
      ...
      {/* Modal si no hay nombre de clienta */}
      {showAgendarPostTicket && !showConfirm && !clientaParaAgendar && (
        <div className="overlay">
          <div className="glass" style={{ width: 380, padding: "28px", textAlign: "center" }}>
            <div style={{ fontSize: "32px", marginBottom: "12px" }}>✓</div>
            <div style={{ fontSize: "18px", fontWeight: 700, marginBottom: "8px" }}>Ticket guardado</div>
            <div style={{ fontSize: "13px", color: "rgba(255,255,255,0.4)", marginBottom: "20px" }}>
              Para agendar cita, registra el nombre de la clienta en el próximo ticket.
            </div>
            <button className="btn-blue" style={{ width: "100%", padding: "12px" }} onClick={() => {
              setShowAgendarPostTicket(false);
              setClientaParaAgendar(null);
            }}>Entendido</button>
          </div>
